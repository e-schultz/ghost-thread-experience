...
export const continuityBridges = [
  {
...
continuityBridges.forEach((bridge, index) => {
  ...
  logData.push({
    id: `cb-${index + 1}`,
    title: `Continuity Bridge: ${bridge.bridge_id}`,
    date: new Date(bridge.metadata.timestamp).toLocaleString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
      hour: '2-digit',
    ...
    content: bridge.content_summary || 
             `Active threads: ${threadNames}\nMode: ${bridge.metadata.mode}\nContext markers: ${bridge.metadata.ctx_markers}`,
    ocrConfidence: 100,
    imagePath: "/lovable-uploads/50184a8f-1f98-4ad7-8da0-02d08a65210a.png",
    bridgeData: bridge,
  });
